...
GDB_URI = f"zip://{GDB_ZIP_PATH.as_posix()}!TELOISE_TELOISE_20250610-130725.gdb"
...
HOSTNAME_PATTERN = re.compile(
    r"\bsysname\s+([^\s]+)|\bhostname\s+([^\s]+)", re.IGNORECASE
)
...
HOUSING_SITE_RELATION_LAYERS = {
    ...
    "SupportStructure__Hubsite": (
        "SupportStructure",
        "SUPPORTSTRUCTURE_MIGRATION_OID",
        "HUBSITE_MIGRATION_OID",
    ),
}
...
ROLE_BY_OFFER_NORMALIZED = {
    norm_text(key): value for key, value in ROLE_BY_OFFER.items()
}
...
def extract_service_refs(*values: object) -> list[str]:
    ...
    for value in values:
        ...
        for left, right in SERVICE_PATTERN.findall(text):
            refs.append(
                f"{norm_text(left).replace(' ', '')}/{norm_text(right).replace(' ', '')}"
            )
    deduped: list[str] = []
    seen = set()
    for ref in refs:
...
def _project_wgs84_to_l93(
    lon: object, lat: object
    ...
    try:
        if shape is not None:
            ...
            if geom.is_empty:
                return None, None, None, None, None, None
            first = (
                geom.boundary.geoms[0]
                if hasattr(geom.boundary, "geoms") and geom.boundary.geoms
                else geom.centroid
            )
            last = (
                geom.boundary.geoms[-1]
                if hasattr(geom.boundary, "geoms") and geom.boundary.geoms
                else geom.centroid
            )
...
def _geometry_to_l93_xy(
    geometry: object, srid: str
) -> tuple[float | None, float | None]:
...
def _line_geometry_to_l93_points(
    ...
    start_x, start_y, end_x, end_y, centroid_x, centroid_y = (
        _extract_line_geometry_points(geometry)
    )
...
HEADER_CLIENT_SITE_RE = re.compile(
    r"CLIENT\s*:\s*(.*?)\s+SITE\s*:\s*(.*?)\s+DSP\s*:", re.IGNORECASE
)

# CO sub-interface patterns (for xconnect / dot1Q parsing)
SUBIF_NAME_RE = re.compile(
    r"^(\S+)\.(\d+)$"
)  # GigabitEthernet0/0/0.2255 → parent, vlan
DOT1Q_RE = re.compile(r"encapsulation\s+dot1[qQ]\s+(\d+)", re.IGNORECASE)
XCONNECT_RE = re.compile(r"xconnect\s+(\S+)\s+(\d+)", re.IGNORECASE)
CO_SITE_CODE_RE = re.compile(
    r"((?:60|80)[a-z]{2,4}\d)", re.IGNORECASE
)  # 60cml1, 80air1
...
def _normalize_city(value: object) -> str:
    tokens = [
        ...
        if token not in ADDRESS_NOISE_STOPWORDS
    ]
...
def parse_address_seed(value: object) -> dict[str, str]:
    ...
    street_idx = next(
        (idx for idx, token in enumerate(tokens) if token in STREET_TYPE_TOKENS), None
    )
    street_name = ""
    city = ""
    if street_idx is not None:
        ...
        if postcode and postcode in normalized:
            ...
            street_tokens = [
                token
                for token in tokens[street_idx:]
                if token != postcode and token not in city_tokens
            ]
        ...
        street_name = " ".join(
            ...
            if token and token not in ADDRESS_NOISE_STOPWORDS
        ).strip()
    elif postcode:
...
def _seed_type_from_parsed(parsed: dict[str, str], preferred_type: str) -> str:
    ...
    if parsed["city"]:
        return (
            "lea_city_only"
            if preferred_type.startswith("lea_")
            else f"{preferred_type}_city_only"
        )
    return preferred_type
...
def _has_business_prefix_before_street(raw_value: str, parsed: dict[str, str]) -> bool:
    ...
    if not prefix:
        return False
    prefix_tokens = [
        token for token in prefix.split() if token not in ADDRESS_NOISE_STOPWORDS
    ]
...
def classify_lea_signal(
    raw_value: object, source_column: str, known_cities: set[str]
    ...
    if _is_noise_label(raw_text):
        ...
        parsed["street_name"]
        and parsed["city"]
        and _has_business_prefix_before_street(raw_text, parsed)
    ):
        ...
        parsed["city"] and parsed["city"] in known_cities and not parsed["street_name"]
    ):
...
def load_network_text_artifacts(con: sqlite3.Connection) -> None:
    ...
    for path in sorted(CONFIG_DIR.rglob("*.txt")):
        ...
        if announcement and not site_hint:
            site_from_announcement = re.search(
                r"SITE\s+(.+?)--", announcement, re.IGNORECASE
            )
            site_hint = (
                site_from_announcement.group(1).strip()
                if site_from_announcement
                else ""
            )
        if announcement and not client_hint:
            client_from_announcement = re.search(
                r"L2 RAD\s+\S+\s+(.+?)--SITE", announcement, re.IGNORECASE
            )
            client_hint = (
                client_from_announcement.group(1).strip()
                if client_from_announcement
                else ""
            )
        ...
        device_records.append(
            (
                ...
                norm_text(
                    " ".join([hostname, announcement, header_info, location_text])
                ),
            )
        )
        ...
        def flush_interface() -> None:
            ...
            interface_counter += 1
            description = " | ".join(
                part for part in current_interface_desc if part
            ).strip()
            interface_records.append(
                (
                    ...
                    description,
                    json.dumps(
                        extract_service_refs(current_interface_name, description),
                        ensure_ascii=True,
                    ),
                    json.dumps(
                        extract_route_refs(current_interface_name, description),
                        ensure_ascii=True,
                    ),
        ...
        def flush_vlan() -> None:
            ...
            vlan_records.append(
                (
                    ...
                    current_vlan_label,
                    json.dumps(
                        extract_service_refs(current_vlan_label), ensure_ascii=True
                    ),
                    json.dumps(
                        extract_route_refs(current_vlan_label), ensure_ascii=True
                    ),
            ...
            for blk in _parse_co_subinterfaces(payload):
                ...
                subif_records.append(
                    (
                        ...
                        json.dumps(
                            extract_route_refs(blk["description"]), ensure_ascii=True
                        ),
                    )
                )

    con.executemany(
        "insert into ref_network_devices values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        device_records,
    )
    con.executemany(
        "insert into ref_network_interfaces values (?,?,?,?,?,?,?,?,?,?,?)",
        interface_records,
    )
    con.executemany(
        "insert into ref_network_vlans values (?,?,?,?,?,?,?,?,?,?)", vlan_records
    )
    if subif_records:
        con.executemany(
            "insert into ref_co_subinterface values (?,?,?,?,?,?,?,?,?,?,?,?)",
            subif_records,
        )
    con.commit()
    LOG.info(
...
def build_party_master(con: sqlite3.Connection) -> None:
    ...
    for row in con.execute(
        "select distinct client_contractant from lea_active_lines where trim(client_contractant) <> ''"
    ):
        party_sources.append((row[0], row[0], "contractant", "lea_active_lines", 100))
    for row in con.execute(
        "select distinct client_final from lea_active_lines where trim(client_final) <> ''"
    ):
        party_sources.append((row[0], row[0], "client_final", "lea_active_lines", 95))
    for row in con.execute(
        "select distinct label from ref_network_vlans where trim(label) <> ''"
    ):
        label = clean_business_label(row[0])
        if label:
            party_sources.append(
                (label, row[0], "network_vlan", "ref_network_vlans", 70)
            )
    for row in con.execute(
        "select distinct description from ref_network_interfaces where trim(description) <> ''"
    ):
        label = clean_business_label(row[0])
        if label:
            party_sources.append(
                (label, row[0], "network_interface", "ref_network_interfaces", 65)
            )
    for row in con.execute(
        "select distinct client_hint from ref_network_devices where trim(client_hint) <> ''"
    ):
        label = clean_business_label(row[0]) or row[0]
        party_sources.append(
            (label, row[0], "device_client_hint", "ref_network_devices", 75)
        )
    ...
    for row in con.execute(
        ...
        if client_hint and len(client_hint) >= 3:
            party_sources.append(
                (client_hint, row[0], "endpoint_client_hint", "lea_active_lines", 80)
            )
    ...
    for row in con.execute(
        ...
        if client_hint and len(client_hint) >= 3:
            party_sources.append(
                (client_hint, row[0], "site_client_hint", "ref_sites", 75)
            )

    priority_order = {
        "contractant": 1,
        "client_final": 2,
        "endpoint_client_hint": 3,
        "site_client_hint": 4,
        "device_client_hint": 5,
        "network_vlan": 6,
        "network_interface": 7,
    }
    ...
    for normalized, items in by_norm.items():
        items.sort(
            key=lambda item: (
                priority_order.get(item[2], 99),
                -item[4],
                len(norm_text(item[0])),
            )
        )
        canonical_name, _, party_type, _, source_priority = items[0]
        party_id = f"PTY-{safe_hash([normalized])}"
        party_records.append(
            (party_id, canonical_name, normalized, party_type, source_priority)
        )
...
def create_schema(con: sqlite3.Connection) -> None:
    con.executescript(
        ...
        drop view if exists v_site_passive_assets;
        drop view if exists v_route_endpoint_sites;
        drop view if exists v_lease_site_pair;
        ...
        create index idx_ref_optical_housing_site on ref_optical_housing(site_id);
        create index idx_ref_optical_housing_migoid on ref_optical_housing(migration_oid);
        create index idx_ref_optical_connection_housing on ref_optical_connection(housing_migration_oid);
        create index idx_ref_optical_connection_obj1 on ref_optical_connection(obj1_migration_oid);
        create index idx_ref_optical_connection_obj2 on ref_optical_connection(obj2_migration_oid);
        create index idx_ref_optical_cable_migoid on ref_optical_cable(migration_oid);
        create index idx_ref_optical_cable_centroid on ref_optical_cable(geom_centroid_x, geom_centroid_y);
        create index idx_ref_optical_lease_endpoint_site on ref_optical_lease_endpoint(site_id);
        create index idx_ref_routes_ref on ref_routes(route_ref);
        create index idx_ref_route_parcours_route_step on ref_route_parcours(route_ref, step_type);
        ...
        create view v_site_passive_assets as
        select
            s.site_id,
            s.reference,
            s.geom_x,
            ...
            count(distinct case when osl.object_type like '%CABLE%' then osl.object_migration_oid end) as cable_link_count,
            count(distinct ep.optical_lease_id) as lease_endpoint_count
        from ref_sites s
        left join ref_optical_housing h on h.site_id = s.site_id
        left join ref_optical_site_link osl on osl.site_id = s.site_id
        left join ref_optical_lease_endpoint ep on ep.site_id = s.site_id
        group by s.site_id, s.reference, s.geom_x, s.geom_y;

        create view v_route_endpoint_sites as
        select
            route_ref,
            route_id,
            step_no,
            ...
            cable_out,
            commentaire
        from ref_route_parcours
        where upper(coalesce(step_type, '')) in ('DEPART', 'ARRIVEE', 'ORIGIN', 'DESTINATION');

        create view v_lease_site_pair as
        select
            a.site_id as site_a_id,
            z.site_id as site_z_id,
            l.optical_lease_id,
            l.ref_exploit,
            l.reference,
            l.lease_kind,
            ...
            l.client,
            l.comments,
            a.site_name as site_a_name,
            z.site_name as site_z_name
        from ref_optical_lease l
        join ref_optical_lease_endpoint a on a.optical_lease_id = l.optical_lease_id and upper(coalesce(a.endpoint_label, '')) in ('A', 'L1')
        join ref_optical_lease_endpoint z on z.optical_lease_id = l.optical_lease_id and upper(coalesce(z.endpoint_label, '')) in ('Z', 'L2')
        where a.site_id is not null and z.site_id is not null and a.site_id <> z.site_id;
        """
    )
    con.commit()
...
def build_grouping_key(
    ...
    if norm_text(contract_file):
        return (
            f"CONTRACT|{safe_hash([client_contractant, contract_file, nature_service])}"
        )
...
def load_ban_addresses(con: sqlite3.Connection) -> None:
    ...
    with BAN_60_PATH.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            ...
            department_code = _extract_ban_value(
                row, "code_departement", "department_code"
            )
            ...
            if not full_label:
                full_label = " ".join(
                    part for part in [house_number, street_name, postcode, city] if part
                ).strip()
    ...
    for record in raw_records:
        if (
            ...
            and record["x_l93"] is not None
            and record["y_l93"] is not None
        ):
            xy_counts[
                (
                    str(record["normalized_city"]),
                    float(record["x_l93"]),
                    float(record["y_l93"]),
                )
            ] += 1
...
def load_sites(con: sqlite3.Connection) -> None:
    ...
    con.executemany(
        "insert into ref_sites values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", records
    )
    con.commit()
...
def load_routes(con: sqlite3.Connection) -> None:
    ...
    for layer, (
        ...
        object_oid_field,
        hubsite_oid_field,
    ) in HOUSING_SITE_RELATION_LAYERS.items():
        for item in iter_gdb_records(layer):
    ...
    for layer in (
        ...
        for item, geometry, srid in iter_gdb_features(layer):
            ...
            parent_type, parent_migration_oid = parent_by_child.get(
                migration_oid, (None, None)
            )
    ...
    for item, geometry, srid in iter_gdb_features("Fiber_Cable"):
        ...
        (
            ...
            geom_centroid_x,
            geom_centroid_y,
        ) = _line_geometry_to_l93_points(geometry, srid)
        cable_records.append(
            (
                ...
                geom_centroid_y,
                "gdb_geometry"
                if geom_centroid_x is not None and geom_centroid_y is not None
                else None,
                srid,
            )
        )
        for token in site_tokens:
            ...
            cable_hint_records.append(
                (cable_id, token, base_score, "fiber_cable_name_token")
            )
...
def load_lease_tables(con: sqlite3.Connection) -> None:
    ...
    def best_site_for_endpoint(
        ...
        for object_kind, migration_oid in candidates:
            if migration_oid and (object_kind, migration_oid) in housing_site_links:
                site_id, site_name, rule_name, score = housing_site_links[
                    (object_kind, migration_oid)
                ]
    ...
    for idx, item in enumerate(iter_gdb_records("LEASE_TEMPLATE"), start=1):
        ...
        for (
            ...
            (
                "L1",
                item.get("HOUSINGTYPEL1"),
                item.get("HOUSINGMIGOIDL1"),
                item.get("TYPEL1"),
                item.get("MIGOIDL1"),
                item.get("L1_CONN1"),
                item.get("L1_CONN2"),
                item.get("REFERENCEL1"),
                1,
                "origin",
            ),
            (
                "L2",
                item.get("HOUSINGTYPEL2"),
                item.get("HOUSINGMIGOIDL2"),
                item.get("TYPEL2"),
                item.get("MIGOIDL2"),
                item.get("L2_CONN1"),
                item.get("L2_CONN2"),
                item.get("REFERENCEL2"),
                2,
                "destination",
            ),
        ):
            site_match = best_site_for_endpoint(
                housing_type,
                housing_migoid,
                object_type,
                object_migoid,
    ...
    con.executemany(
        "insert into ref_lease_template values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        lease_template_records,
    )
    ...
    for idx, item in enumerate(iter_gdb_records("Fiber_Lease"), start=1):
        base_lease_id = str(
            item.get("MIGRATION_OID")
            or f"FLEASE-{item.get('OID')}-{item.get('STARTRANGE')}-{item.get('ENDRANGE')}"
        )
        ...
        route_ref = str(
            item.get("REF_EXPLOIT") or ""
        ).strip().upper() or infer_route_ref(
            item.get("REFERENCE"), item.get("COMMENTS")
        )
        optical_lease_records.append(
            (
                lease_id,
    ...
    con.executemany(
        "insert into ref_fiber_lease values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        fiber_lease_records,
    )
    ...
    for idx, item in enumerate(iter_gdb_records("ISPLease"), start=1):
        base_lease_id = str(
            item.get("MIGRATION_OID")
            or f"ISPLEASE-{item.get('OID')}-{item.get('ISPCONTAINERID')}"
        )
        lease_id = f"{base_lease_id}-{idx:06d}"
    ...
    con.executemany(
        "insert into ref_isp_lease values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        isp_lease_records,
    )
    con.executemany(
...
def load_swag_interfaces(con: sqlite3.Connection) -> None:
    ...
    con.executemany(
        "insert into ref_swag_interfaces values (?,?,?,?,?,?,?,?,?,?)", records
    )
    con.commit()
...
def load_cpe_inventory(con: sqlite3.Connection) -> None:
    ...
    con.executemany(
        "insert into ref_cpe_inventory values (?,?,?,?,?,?,?,?,?,?,?)", records
    )
    con.commit()
...
def build_service_master(con: sqlite3.Connection) -> None:
    ...
    for service_key, members in groups.items():
        ...
        for member in members:
            service_bss_records.append(
                (
                    service_id,
                    member[0],
                    member[4],
                    1 if member[0] == principal[0] else 0,
                )
            )
    ...
    con.executemany(
        "insert into service_bss_line values (?,?,?,?)", service_bss_records
    )
    con.commit()
...
def reconcile_iru_maintenance(con: sqlite3.Connection) -> None:
    ...
    if not maintenance_only:
        LOG.info(
            "IRU maintenance reconciliation: no maintenance-only services found, nothing to do"
        )
        return
    ...
    for m_sid, m_info in maintenance_only.items():
        ...
        if matched_principal:
            ...
            con.execute(
                "DELETE FROM service_master_active WHERE service_id = ?", (m_sid,)
            )
...
def build_site_index(
    con: sqlite3.Connection,
) -> tuple[dict[str, list[tuple]], list[tuple]]:
    rows = con.execute(
...
def _best_ban_match(
    con: sqlite3.Connection, parsed: dict[str, str]
) -> dict[str, object] | None:
...
def build_service_lea_signals(
    ...
    known_cities = {
        row[0]
        for row in con.execute(
            "select distinct normalized_city from ref_ban_address where normalized_city <> ''"
        ).fetchall()
        if row[0]
    }
    ...
    for row in service_rows:
        ...
        spatial_hints_blob = (
            row["spatial_hints_json"] if "spatial_hints_json" in row.keys() else None
        )
...
def build_service_spatial_seeds(
    con: sqlite3.Connection,
) -> dict[str, list[dict[str, object]]]:
    ...
    def append_seed(service_id: str, signal: dict[str, object]) -> None:
        ...
        effective_score = (
            min(raw_ban_score, int(signal["signal_score"]), xy_discriminance_score)
            if ban_match
            else 0
        )
        row = (
        ...
        seeds_by_service[service_id].append(
            {
                ...
                "xy_precision_class": ban_match["xy_precision_class"]
                if ban_match
                else None,
                "xy_discriminance_score": ban_match["xy_discriminance_score"]
                if ban_match
                else None,
                "same_xy_count_in_city": ban_match["same_xy_count_in_city"]
                if ban_match
                else None,
                "is_reused_xy": ban_match["is_reused_xy"] if ban_match else 0,
                "is_heavily_reused_xy": ban_match["is_heavily_reused_xy"]
                if ban_match
                else 0,
            }
        )
    ...
    for service_id, signals in signals_by_service.items():
        strong = [
            ...
            if signal["is_ban_candidate"]
            and not signal["is_noise"]
            and signal["semantic_strength"] == "strong"
        ]
        medium = [
            signal
            for signal in signals
            if signal["is_ban_candidate"]
            and not signal["is_noise"]
            and signal["semantic_strength"] == "medium"
        ]
        weak = [
            signal
            for signal in signals
            if signal["is_ban_candidate"]
            and not signal["is_noise"]
            and signal["semantic_strength"] == "weak"
        ]
        selected = (
            sorted(
                strong,
                key=lambda item: (item["source_priority"], item["signal_score"]),
                reverse=True,
            )[:3]
            + sorted(
                medium,
...
def _best_spatial_site_match(
    ...
    for seed in _select_spatial_seeds_for_endpoint(seeds, endpoint_label):
        ...
        for row in all_sites:
            ...
            adjusted_score = min(
                raw_score,
                int(seed.get("xy_discriminance_score") or 0),
                int(seed.get("match_score") or 0),
            )
            if adjusted_score <= 0:
                continue
            candidate = {
            ...
            if (
                best is None
                or candidate["adjusted_score"] > best["adjusted_score"]
                or (
                    candidate["adjusted_score"] == best["adjusted_score"]
                    and (candidate["distance_meters"] or 0)
                    < (best["distance_meters"] or 0)
                )
            ):
                best = candidate
    return best
        ...
        for point_x, point_y in object_points:
            ...
            if (
                ...
                or (
                    ...
                    and (
                        best["distance_meters"] is None
                        or distance < best["distance_meters"]
                    )
                )
            ):
                best = {
...
def match_site(
    raw_value: str, alias_index: dict[str, list[tuple]], all_sites: list[tuple]
) -> SiteMatch:
    normalized = norm_text(raw_value)
    if not normalized:
    ...
    if best_row:
        return SiteMatch(
            best_row[0], best_score, "site_token_overlap", best_row[2] or best_row[3]
        )
    return SiteMatch(None, 0, None, None)
...
def reconcile_services(con: sqlite3.Connection) -> None:
    ...
    party_alias_index = {
        row[0]: row[1]
        for row in con.execute("select normalized_alias, party_id from party_alias")
    }
    cpe_by_hostname = {
        norm_text(row[1]): (row[0], row[1])
        for row in con.execute("select cpe_id, hostname from ref_cpe_inventory")
    }

    route_index = {
        (row[1] or "").strip().upper(): row
        for row in con.execute(
            "select route_id, route_ref, network, client, lessee, status from ref_routes"
        )
        if row[1]
    }
    ...
    for row in lease_rows:
        ...
        if (
            site_l1_row
            and site_l2_row
            and site_l1_row["site_id"]
            and site_l2_row["site_id"]
        ):
            site_l1 = SiteMatch(
                site_l1_row["site_id"],
        ...
        if site_l1.site_id and site_l2.site_id:
            lease_pair_index[tuple(sorted([site_l1.site_id, site_l2.site_id]))].append(
                (row, site_l1, site_l2)
            )
    ...
    for row in optical_lease_rows:
        for key in {
            ...
            infer_route_ref(row[3], row[11]) or "",
        } - {""}:
    ...
    cpe_rows = list(
        con.execute(
            "select cpe_id, hostname, normalized_hostname from ref_cpe_inventory"
        )
    )
    ...
    def _relevant_site_signals(
        service_id: str, endpoint_label: str
    ) -> list[sqlite3.Row]:
        preferred_source = f"endpoint_{endpoint_label.lower()}_raw"
        rows = [
            row
            for row in signals_by_service.get(service_id, [])
    ...
    for service in services:
        ...
        for label, raw_value in [("A", endpoint_a_raw), ("Z", endpoint_z_raw)]:
            ...
            signal_matches = [
                (row, match) for row, match in signal_matches if match.site_id
            ]
            ...
            if signal_matches:
                ...
                if best_signal_match.score > match.score or (
                    best_signal_match.score == match.score
                    and best_signal_row["signal_score"] > 0
                ):
                    match = best_signal_match
            spatial_match = _best_spatial_site_match(
                label, seeds_by_service.get(service_id, []), all_sites
            )
            chosen_match = match
            spatial_score = 0
            ...
            if spatial_match is not None:
                ...
                spatial_distance = (
                    round(spatial_match["distance_meters"], 1)
                    if spatial_match["distance_meters"] is not None
                    else None
                )
                spatial_rule = spatial_match["rule"]
                spatial_seed_precision_class = spatial_match["seed"].get(
                    "xy_precision_class"
                )
                spatial_seed_reused_xy_count = spatial_match["seed"].get(
                    "same_xy_count_in_city"
                )
                spatial_context = {
                    ...
                    "seed_discriminance_score": spatial_match["seed"].get(
                        "xy_discriminance_score"
                    ),
                }
                ...
                if (
                    ...
                    or (
                        spatial_match["site_id"] == chosen_match.site_id
                        and spatial_adjusted_score > chosen_match.score
                    )
            ...
            endpoint_records.append(
                (
                    ...
                    best_signal_row["signal_kind"]
                    if best_signal_row is not None
                    else None,
                    best_signal_row["signal_score"]
                    if best_signal_row is not None
                    else None,
                    best_signal_row["raw_value"]
                    if best_signal_row is not None
                    else None,
                ...
                evidence_records.append(
                    build_evidence(
                        ...
                        chosen_match.site_id,
                        {
                            ...
                            "raw_value": raw_value,
                            "site_name": chosen_match.site_name,
                        },
                    )
                )
        ...
        for route_ref in route_refs:
            ...
            logical_route_id = (
                logical_route[0] if logical_route else (route[0] if route else None)
            )
            ...
            append_optical_record(
                ...
                optical_context={
                    "source_layer": logical_route[2]
                    if logical_route
                    else "legacy_route_ref",
            ...
            for lease in lease_by_ref.get(route_ref, []):
                append_optical_record(
                    ...
                    site_z_optical_id=site_z_id,
                    optical_context={
                        "reference_l1": lease[5],
                        "reference_l2": lease[6],
                    },
                )
                evidence_records.append(
                    build_evidence(
            ...
            for interface in network_interface_by_route.get(route_ref, []):
                evidence_records.append(
                    build_evidence(
                        ...
                        {
                            "route_ref": route_ref,
                            "device_name": interface[2],
                            "interface_name": interface[5],
                ...
                evidence_records.append(
                    build_evidence(
                        ...
                        config[0],
                        {
                            "route_ref": route_ref,
                            "hostname": config[2],
                            "header_info": config[3],
                            "vlans": json.loads(config[6] or "[]"),
                        },
                    )
                )
        ...
        _has_valid_route = (
            any(
                route_index.get(rr) or logical_route_by_ref.get(rr) for rr in route_refs
            )
            if route_refs
            else False
        )
        ...
        if _try_site_pair_lease:
            if site_a_id and site_z_id:
                for lease, site_l1, site_l2 in lease_pair_index.get(
                    tuple(sorted([site_a_id, site_z_id])), []
                ):
                    ...
                    route_id = (
                        route[0]
                        if route
                        else (logical_route[0] if logical_route else None)
                    )
                    append_optical_record(
                        service_id,
                        route_ref=route_ref,
                        ...
                        support_type="lease",
                        support_ref=lease[0],
                        logical_route_id=logical_route[0]
                        if logical_route
                        else route_id,
                        site_a_optical_id=site_a_id,
                        site_z_optical_id=site_z_id,
        ...
        if True:
            site_tokens = set(
                extract_place_tokens(
                    endpoint_a_raw, endpoint_z_raw, client_final, principal_external_ref
                )
            )
            technical_signals = [
                row
            ...
            for cable in candidate_cables.values():
                ...
                append_optical_record(
                    ...
                    spatial_match_rule="address_to_cable_endpoint"
                    if cable_spatial_adjusted
                    else None,
                    spatial_distance_meters=round(cable_distance, 1)
                    if cable_distance is not None
                    else None,
                    spatial_score=cable_spatial_score or None,
                    origin_signal_kind=origin_signal["signal_kind"]
                    if origin_signal is not None
                    else None,
                    origin_signal_score=origin_signal["signal_score"]
                    if origin_signal is not None
                    else None,
                    origin_signal_raw_value=origin_signal["raw_value"]
                    if origin_signal is not None
                    else None,
                    spatial_adjusted_score=cable_spatial_adjusted or None,
                    spatial_seed_precision_class=cable_seed.get("xy_precision_class"),
                    spatial_seed_reused_xy_count=cable_seed.get(
                        "same_xy_count_in_city"
                    ),
                )
                if cable_spatial_adjusted:
            ...
            if site_a_id:
                for housing in housing_by_site.get(site_a_id, []):
                    ...
                    append_optical_record(
                        ...
                        spatial_match_rule="address_to_housing"
                        if housing_spatial_adjusted
                        else None,
                        spatial_distance_meters=round(housing_distance, 1)
                        if housing_distance is not None
                        else None,
                        ...
                        spatial_seed_precision_class=housing_seed.get(
                            "xy_precision_class"
                        ),
                        spatial_seed_reused_xy_count=housing_seed.get(
                            "same_xy_count_in_city"
                        ),
                    )
            ...
            for interface in network_interface_by_service.get(service_ref, []):
                ...
                network_records.append(
                    (
                        ...
                        None,
                        None,
                        json.dumps(vlan_ids, ensure_ascii=True),
                    )
                )
                evidence_records.append(
                    build_evidence(
            ...
            for config in config_by_service.get(service_ref, []):
                ...
                network_records.append(
                    (
                        ...
                        None,
                        cpe_id,
                        "config_hostname_to_cpe" if cpe_id else None,
                        cpe_score,
                        config[0],
                        "config_service_ref_exact",
                        95,
                        json.dumps(inferred_vlans, ensure_ascii=True),
                    )
                )
                evidence_records.append(
                    build_evidence(
            ...
            for device in network_device_by_service.get(service_ref, []):
                ...
                network_records.append(
                    (
                        ...
                        None,
                        cpe_id,
                        "device_hostname_to_cpe" if cpe_id else None,
                        90 if cpe_id else None,
                        None,
                        None,
                        None,
                        json.dumps(inferred_vlans, ensure_ascii=True),
        ...
        if nature_service == "Lan To Lan":
            ...
            if not _inserted_vlans:
                ...
                for _tok in sorted(_vlan_search_tokens, key=len, reverse=True):
                    _vlan_matches = [
                        r
                        for r in network_vlan_rows
                        if r[6] and _tok in norm_text(r[6]).split()
                    ]
                    ...
                    if 2 <= len(_vlan_matches) <= 3:
                        _unique_clean = {
                            clean_business_label(m[6]) for m in _vlan_matches
                        }
            ...
            for _irow in network_interface_rows:
                _iscore = score_label_match(
                    service_seeds, _irow[6]
                )  # index 6 = description
            ...
            for _drow_raw in network_device_rows:
                _dcombined = " ".join(
                    part for part in [_drow_raw[7], _drow_raw[8], _drow_raw[5]] if part
                )
                ...
                if _dscore >= 72 and _drow_raw[0] not in _inserted_devices:
                    ...
                    network_records.append(
                        (
                            ...
                            None,
                            _dcpe_id,
                            "device_label_to_cpe" if _dcpe_id else None,
                            _dscore if _dcpe_id else None,
                            None,
                            None,
                            None,
                            _drow_raw[11] or json.dumps([], ensure_ascii=True),
                        )
                    )
                    evidence_records.append(
                        build_evidence(
                            ...
                            _drow_raw[0],
                            {
                                "device_name": _drow_raw[1],
                                "client_hint": _drow_raw[7],
                                "site_hint": _drow_raw[8],
                                "announcement": _drow_raw[5],
                            },
                        )
                    )
                    _dparty_id = party_id_for_alias(
                        party_alias_index, _drow_raw[7] or _drow_raw[8]
                    )
                    if _dparty_id:
                        service_party_records.append(
                            (
    ...
    con.executemany(
        "insert into service_party values (?,?,?,?,?,?,?)", service_party_records
    )
    if optical_records:
    ...
    con.executemany(
        "insert into service_match_evidence values (?,?,?,?,?,?,?,?)", evidence_records
    )
    con.executemany(
...
def build_publication_views(con: sqlite3.Connection) -> None:
    ...
    for row in con.execute(
        "select service_id, role_name, party_id, rule_name, score, source_table, source_key from service_party"
    ):
    ...
    for service_id, score in con.execute(
        "select service_id, score from service_match_evidence"
    ):
    ...
    for (
        ...
        contract_party_row = best_scored_row(
            party_rows.get((service_id, "contract_party"), []), [4]
        )
        final_party_row = best_scored_row(
            party_rows.get((service_id, "final_party"), []), [4]
        )
        ...
        network_row = best_scored_row(
            network_rows.get(service_id, []), [4, 7, 10, 13, 16]
        )

        best_network_score = (
            max((network_row[index] or 0) for index in [4, 7, 10, 13, 16])
            if network_row
            else 0
        )
        ...
        confidence_score = max(
            best_optical_score, best_network_score, endpoint_a_score, endpoint_z_score
        )
                ...
                json.dumps(
                    {
                        ...
                        "endpoint_a_spatial_score": endpoint_a_row[7]
                        if endpoint_a_row and len(endpoint_a_row) > 7
                        else None,
                        "endpoint_z_spatial_score": endpoint_z_row[7]
                        if endpoint_z_row and len(endpoint_z_row) > 7
                        else None,
                        "endpoint_a_spatial_adjusted_score": endpoint_a_row[14]
                        if endpoint_a_row and len(endpoint_a_row) > 14
                        else None,
                        "endpoint_z_spatial_adjusted_score": endpoint_z_row[14]
                        if endpoint_z_row and len(endpoint_z_row) > 14
                        else None,
                        "endpoint_a_signal_kind": endpoint_a_row[11]
                        if endpoint_a_row and len(endpoint_a_row) > 11
                        else None,
                        "endpoint_z_signal_kind": endpoint_z_row[11]
                        if endpoint_z_row and len(endpoint_z_row) > 11
                        else None,
                        "logical_route_id": optical_row[17]
                        if optical_row and len(optical_row) > 17
                        else None,
                        "cable_id": optical_row[18]
                        if optical_row and len(optical_row) > 18
                        else None,
                        "housing_id": optical_row[21]
                        if optical_row and len(optical_row) > 21
                        else None,
                        "optical_site_a": optical_row[24]
                        if optical_row and len(optical_row) > 24
                        else None,
                        "optical_site_z": optical_row[25]
                        if optical_row and len(optical_row) > 25
                        else None,
                        "optical_reasoning": optical_context,
                        "optical_spatial_match_rule": optical_row[27]
                        if optical_row and len(optical_row) > 27
                        else None,
                        "optical_spatial_distance_meters": optical_row[28]
                        if optical_row and len(optical_row) > 28
                        else None,
                        "optical_spatial_score": optical_row[29]
                        if optical_row and len(optical_row) > 29
                        else None,
                        "optical_origin_signal_kind": optical_row[30]
                        if optical_row and len(optical_row) > 30
                        else None,
                        "optical_spatial_adjusted_score": optical_row[33]
                        if optical_row and len(optical_row) > 33
                        else None,
                    },
                    ensure_ascii=True,
                    sort_keys=True,
        ...
        if not contract_party_row:
            issues.append(
                ("missing_contract_party", "high", "Contract party unresolved")
            )
        ...
        if nature_service == "Lan To Lan" and best_network_score < 72:
            issues.append(
                (
                    ...
                    "high",
                    "No reliable network support matched",
                )
            )
        if nature_service in {"IRU FON", "Location FON"} and best_optical_score < 80:
            issues.append(
                (
                    ...
                    "high",
                    "No reliable optical support matched",
                )
            )
        if match_state != "auto_valid":
            issues.append(
                ("manual_review", "medium", "Service requires analyst review")
            )
    ...
    con.executemany(
        "insert into gold_service_active values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        gold_rows,
    )
    con.executemany(
        "insert into service_review_queue values (?,?,?,?,?,?,?)", review_rows
    )
    con.commit()
    LOG.info(
        "Built %s gold services and %s review items", len(gold_rows), len(review_rows)
    )
...
def _party_name_by_id(con: sqlite3.Connection) -> dict[str, str]:
    return {
        ...
        for row in con.execute(
            "select party_id, canonical_name from party_master"
        ).fetchall()
    }
...
def _latest_validated_agent_resolution_map(
    con: sqlite3.Connection,
...
def _bss_link_map(con: sqlite3.Connection) -> dict[str, dict[str, object]]:
    ...
    for service_id, members in grouped.items():
        principal = next(
            (row for row in members if row["is_principal"] == 1), members[0]
        )
        links[service_id] = {
            "principal_lea_line_id": principal["lea_line_id"],
            "lea_line_ids_json": json.dumps(
                [row["lea_line_id"] for row in members], ensure_ascii=True
            ),
        }
    return links
...
def _site_geometry_by_id(
    ...
    return {
        ...
        for row in con.execute(
            "select site_id, geom_x, geom_y from ref_sites"
        ).fetchall()
    }
...
def _site_name_for_id(
    site_id: str | None, site_name_by_id: dict[str, str]
) -> str | None:
    if not site_id:
        return None
...
def build_facturable_publication(con: sqlite3.Connection) -> None:
    ...
    try:
        ...
        services = con.execute(
            "select * from service_master_active order by service_id"
        ).fetchall()
        gold_rows = {
            row["service_id"]: row
        ...
        validated_agent = {
            service_id: row
            for service_id, row in latest_agent.items()
            if row["status"] == "validated"
        }
        ...
        for service in services:
            ...
            if (
                validated_res is not None
                and (validated_res["party_final_id"] or "").strip()
            ):
            ...
            if (
                validated_res is not None
                and "resolved_site_a_id" in validated_res.keys()
                and (validated_res["resolved_site_a_id"] or "").strip()
            ):
                ...
                resolved_id, resolved_name, is_unique = _resolve_agent_site(
                    validated_res["site_a"], site_candidates
                )
            ...
            if (
                validated_res is not None
                and "resolved_site_z_id" in validated_res.keys()
                and (validated_res["resolved_site_z_id"] or "").strip()
            ):
                ...
                resolved_id, resolved_name, is_unique = _resolve_agent_site(
                    validated_res["site_z"], site_candidates
                )
                if resolved_id and is_unique:
            ...
            optical_source = (
                "gold"
                if any((route_ref, route_id, lease_id, fiber_lease_id, isp_lease_id))
                else "missing"
            )
            ...
            if validated_res is not None and any(
                (
                    (
                        "route_ref" in validated_res.keys()
                        and (validated_res["route_ref"] or "").strip()
                    ),
                    (
                        "route_id" in validated_res.keys()
                        and (validated_res["route_id"] or "").strip()
                    ),
                    (
                        "lease_id" in validated_res.keys()
                        and (validated_res["lease_id"] or "").strip()
                    ),
                    (
                        "fiber_lease_id" in validated_res.keys()
                        and (validated_res["fiber_lease_id"] or "").strip()
                    ),
                    (
                        "isp_lease_id" in validated_res.keys()
                        and (validated_res["isp_lease_id"] or "").strip()
                    ),
                    (
                        "cable_id" in validated_res.keys()
                        and (validated_res["cable_id"] or "").strip()
                    ),
                    (
                        "housing_id" in validated_res.keys()
                        and (validated_res["housing_id"] or "").strip()
                    ),
                )
            ):
                route_ref = (
                    validated_res["route_ref"]
                    if "route_ref" in validated_res.keys()
                    else route_ref
                )
                route_id = (
                    validated_res["route_id"]
                    if "route_id" in validated_res.keys()
                    else route_id
                )
                lease_id = (
                    validated_res["lease_id"]
                    if "lease_id" in validated_res.keys()
                    else lease_id
                )
                fiber_lease_id = (
                    validated_res["fiber_lease_id"]
                    if "fiber_lease_id" in validated_res.keys()
                    else fiber_lease_id
                )
                isp_lease_id = (
                    validated_res["isp_lease_id"]
                    if "isp_lease_id" in validated_res.keys()
                    else isp_lease_id
                )
                logical_route_id = (
                    validated_res["route_id"]
                    if "route_id" in validated_res.keys() and validated_res["route_id"]
                    else logical_route_id
                )
                cable_id = (
                    validated_res["cable_id"]
                    if "cable_id" in validated_res.keys()
                    else cable_id
                )
                housing_id = (
                    validated_res["housing_id"]
                    if "housing_id" in validated_res.keys()
                    else housing_id
                )
                ...
                validated_res is not None
                and (validated_res["optical_support_ref"] or "").strip()
            ):
                ...
                if remapped is not None:
                    ...
                    if _get_row_value(remapped, "optical_context_json"):
                        try:
                            optical_context = json.loads(
                                _get_row_value(remapped, "optical_context_json") or "{}"
                            )
                        except json.JSONDecodeError:
                            optical_context = {}
                    optical_source = "agent_validated"
            ...
            if isinstance(optical_context, dict):
                spatial_best_distance = optical_context.get(
                    "spatial_distance_meters"
                ) or gold_summary.get("optical_spatial_distance_meters")

            interface_id = gold["interface_id"] if gold is not None else None
            network_interface_id = (
                gold["network_interface_id"] if gold is not None else None
            )
            ...
            if validated_res is not None and any(
                (
                    (
                        "network_interface_id" in validated_res.keys()
                        and (validated_res["network_interface_id"] or "").strip()
                    ),
                    (
                        "network_vlan_id" in validated_res.keys()
                        and (validated_res["network_vlan_id"] or "").strip()
                    ),
                    (
                        "cpe_id" in validated_res.keys()
                        and (validated_res["cpe_id"] or "").strip()
                    ),
                    (
                        "config_id" in validated_res.keys()
                        and (validated_res["config_id"] or "").strip()
                    ),
                    (
                        "inferred_vlans_json" in validated_res.keys()
                        and (validated_res["inferred_vlans_json"] or "").strip()
                    ),
                )
            ):
                network_interface_id = (
                    validated_res["network_interface_id"]
                    if "network_interface_id" in validated_res.keys()
                    else network_interface_id
                )
                network_vlan_id = (
                    validated_res["network_vlan_id"]
                    if "network_vlan_id" in validated_res.keys()
                    else network_vlan_id
                )
                cpe_id = (
                    validated_res["cpe_id"]
                    if "cpe_id" in validated_res.keys()
                    else cpe_id
                )
                config_id = (
                    validated_res["config_id"]
                    if "config_id" in validated_res.keys()
                    else config_id
                )
                inferred_vlans_json = (
                    validated_res["inferred_vlans_json"]
                    if "inferred_vlans_json" in validated_res.keys()
                    and validated_res["inferred_vlans_json"]
                    else inferred_vlans_json
                )
                network_source = "agent_validated"
                note_parts.append("network from agent")
            elif (
                validated_res is not None
                and (validated_res["network_support_id"] or "").strip()
            ):
                ...
                if not any(
                    ...
                    gap_flags.append("agent_hint_unmapped")
            if not any(
                (interface_id, network_interface_id, network_vlan_id, cpe_id, config_id)
            ):
            ...
            critical_gaps = _critical_gap_flags(
                service["nature_service"], unique_gap_flags
            )

            if selected_truth_source == "agent_validated" and not critical_gaps:
                publication_status = "published_validated"
            elif (
                selected_truth_source in {"gold_auto_valid", "gold_review_required"}
            ...
            if (
                selected_truth_source.startswith("gold")
                and publication_status == "published_with_gaps"
                and not note_parts
            ):
            ...
            spatial_confidence_band = _spatial_confidence_from_distance(
                float(spatial_best_distance)
                if spatial_best_distance is not None
                else None
            )
            spatial_summary = {
                "seed_a": {
                    "label": spatial_seed_a["raw_value"]
                    if spatial_seed_a is not None
                    else None,
                    "source": spatial_seed_a["seed_type"]
                    if spatial_seed_a is not None
                    else None,
                    "signal_kind": spatial_seed_a["source_signal_kind"]
                    if spatial_seed_a is not None
                    else None,
                    "semantic_strength": spatial_seed_a["source_semantic_strength"]
                    if spatial_seed_a is not None
                    else None,
                    "city": spatial_seed_a["city_hint"]
                    if spatial_seed_a is not None
                    else None,
                    "x_l93": spatial_seed_a["x_l93"]
                    if spatial_seed_a is not None
                    else None,
                    "y_l93": spatial_seed_a["y_l93"]
                    if spatial_seed_a is not None
                    else None,
                    "xy_precision_class": spatial_seed_a["xy_precision_class"]
                    if spatial_seed_a is not None
                    else None,
                    "same_xy_count_in_city": spatial_seed_a["same_xy_count_in_city"]
                    if spatial_seed_a is not None
                    else None,
                },
                "seed_z": {
                    "label": spatial_seed_z["raw_value"]
                    if spatial_seed_z is not None
                    else None,
                    "source": spatial_seed_z["seed_type"]
                    if spatial_seed_z is not None
                    else None,
                    "signal_kind": spatial_seed_z["source_signal_kind"]
                    if spatial_seed_z is not None
                    else None,
                    "semantic_strength": spatial_seed_z["source_semantic_strength"]
                    if spatial_seed_z is not None
                    else None,
                    "city": spatial_seed_z["city_hint"]
                    if spatial_seed_z is not None
                    else None,
                    "x_l93": spatial_seed_z["x_l93"]
                    if spatial_seed_z is not None
                    else None,
                    "y_l93": spatial_seed_z["y_l93"]
                    if spatial_seed_z is not None
                    else None,
                    "xy_precision_class": spatial_seed_z["xy_precision_class"]
                    if spatial_seed_z is not None
                    else None,
                    "same_xy_count_in_city": spatial_seed_z["same_xy_count_in_city"]
                    if spatial_seed_z is not None
                    else None,
                },
            ...
            rows_to_insert.append(
                (
                    ...
                    spatial_seed_a["xy_precision_class"]
                    if spatial_seed_a is not None
                    else None,
                    spatial_seed_z["xy_precision_class"]
                    if spatial_seed_z is not None
                    else None,
                    spatial_seed_a["same_xy_count_in_city"]
                    if spatial_seed_a is not None
                    else None,
                    spatial_seed_z["same_xy_count_in_city"]
                    if spatial_seed_z is not None
                    else None,
        ...
        placeholders = ", ".join(
            "?" for _ in range(len(rows_to_insert[0]) if rows_to_insert else 0)
        )
        con.executemany(
...
def build_report(con: sqlite3.Connection) -> None:
    ...
    total_services = con.execute(
        "select count(*) from service_master_active"
    ).fetchone()[0]
    ...
    network_devices = con.execute(
        "select count(*) from ref_network_devices"
    ).fetchone()[0]
    network_vlans = con.execute("select count(*) from ref_network_vlans").fetchone()[0]
    final_rows = (
        con.execute("select count(*) from service_facturable_final").fetchone()[0]
        if _table_exists(con, "service_facturable_final")
        else 0
    )
    final_status_rows = (
        con.execute(
    ...
    final_with_party = (
        con.execute(
            "select count(*) from service_facturable_final where final_party_id is not null and trim(final_party_id) <> ''"
        ).fetchone()[0]
        if final_rows
        else 0
    )
    final_with_site_a = (
        con.execute(
            "select count(*) from service_facturable_final where site_a_id is not null and trim(site_a_id) <> ''"
        ).fetchone()[0]
        if final_rows
        else 0
    )
    final_with_site_z = (
        con.execute(
            "select count(*) from service_facturable_final where site_z_id is not null and trim(site_z_id) <> ''"
        ).fetchone()[0]
        if final_rows
        else 0
    )
